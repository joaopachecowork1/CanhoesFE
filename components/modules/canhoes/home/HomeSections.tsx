    ...
    <Card className={HERO_CARD_CLASS}>
      <CardContent className="py-8">
        <ErrorAlert
          ...
          description={errorMessage ?? productHomeCopy.errorDescription}
...
export function CanhoesEventHomeContent({
  ...
  viewModel: CanhoesEventHomeViewModel;
}>) {
  const {
    event,
    homeCopy,
  ...
  const metricCards = [
    ...
    {
      hint: secretSanta.hasAssignment
  ...
  const checklistItems = [
    {
      ...
      hint: overview.modules.wishlist
    ...
    },
    {
      ...
      hint: overview.permissions.canSubmitProposal
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.55fr)_minmax(0,1fr)]">
        {overview.modules.feed ? (
          ...
          >
            {recentPosts.length === 0 ? (
              <HomePanelState>{productHomeCopy.emptyFeed}</HomePanelState>
            ) : (
        ...
        <div className="space-y-4">
          {overview.modules.secretSanta ? (
            <HomePanel title={productHomeCopy.secretSantaTitle} icon={Gift}>
          ...
          <HomePanel title={productHomeCopy.checklistTitle} icon={Vote}>
