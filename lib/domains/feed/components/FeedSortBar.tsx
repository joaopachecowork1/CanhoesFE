    ...
    <div className="flex items-center gap-2 rounded-[var(--radius-xl-token)] border border-[rgba(255,255,255,0.2)] bg-[rgba(18,24,12,0.94)] px-3 py-2 text-[var(--text-primary)] shadow-[var(--shadow-elevation-sm)]">
      ...
      {HUB_FEED_SORT_OPTIONS.map((option) => (
        <button
          ...
          className={cn(
            "bg-white/5 text-[var(--text-primary)] border border-white/[0.18] transition-all duration-200 hover:bg-white/[0.14] rounded-full px-3 py-1 text-xs font-medium transition-all duration-200",
            sort === option.value ? "bg-white/20 text-[var(--text-primary)] border-[#C8A46B] font-bold shadow-[0_0_0_1px_rgba(234,179,8,0.3)]" : ""
          )}
        >
          {option.label}
        </button>
